...
int solveTD(vector<int> &nums,int start,int end,vector<vector<int>> &dp){
    ...
    int coins=INT_MIN;
    for(int i=start;i<=end;i++){
        coins=max(coins,nums[start-1]*nums[i]*nums[end+1] + solveTD(nums,start,i-1,dp) + solveTD(nums,i+1,end,dp));
    }
    return dp[start][end]=coins;
}

int maxCoinsTD(vector<int> &nums){
...
int solveBU(vector<int> &nums){
    ...
    for(int start=n;start>=1;--start){
        for(int end=start;end<=n;++end){   //important step
            int coins=INT_MIN;
            for(int i=start;i<=end;i++){
